...
class SensorUtilsTest {
  ...
  void log_a_single_info_summary_when_only_some_files_are_not_found() {
    ...
    assertThat(logTester.logs(Level.INFO)).contains(
            "Coverage report '" + MANY_FILES_REPORT + "': 35 of 36 files were not found in the analysed sources of 'my-module'."
  ...
  void aggregate_log_warning_after_all_ambiguous_report_entries_are_skipped() {
    ...
    String ambiguitySummary =
            "Coverage report '" + SINGLE_FILE_REPORT + "': coverage data was skipped for 1 of 1 files because their paths could not be resolved unambiguously in 'my-project'."
                    + " Enable debug logs for the full list.";
  ...
  void warn_when_every_report_entry_is_unresolved_for_different_reasons() {
    ...
    assertThat(logTester.logs(Level.INFO)).containsExactly(
            ...
            "Coverage report '" + MANY_FILES_REPORT + "': coverage data was skipped for 1 of 36 files because their paths could not be resolved unambiguously in 'my-project'."
                    + " Enable debug logs for the full list.");
    assertThat(logTester.logs(Level.WARN)).containsExactly(
            "None of the 36 files in coverage report '" + MANY_FILES_REPORT + "' could be matched to the analysed sources of 'my-project'."
  ...
  void warn_once_when_no_file_of_a_report_is_found() {
    ...
    assertThat(logTester.logs(Level.WARN)).containsExactly(
            "None of the 1 files in coverage report '" + SINGLE_FILE_REPORT + "' could be matched to the analysed sources of 'my-module'."
                    + " No coverage was imported from this report.");
    verify(analysisWarnings).addUnique(SensorUtils.NOTHING_MATCHED_ANALYSIS_WARNING);
  }
